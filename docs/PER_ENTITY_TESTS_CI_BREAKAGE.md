...
(depois de mexer no cache do Spike — ver `docs/MEMORY_ARCHITECTURE.md` — os
dois problemas abaixo já existiam antes, só nunca tinham sido notados por
alguém rodando esse step até o fim).
...
`docs/MEMORY_ARCHITECTURE.md`), `rv32i3stage_core_sim_test.vhd` instancia
...
`asm`/`c` — ver `docs/DATA_HARVARD_BUG.md` — só que adaptado ao formato de
config único deste suite). `tests/python/runner.py` agora pula qualquer
entrada com `"skip": true` quando roda `all` (imprimindo o motivo), mas
ainda tenta rodar normalmente se chamada explicitamente por nome
(`python3 tests/python/runner.py one`) — útil para quem for atacar o
