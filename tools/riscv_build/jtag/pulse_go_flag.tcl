# Writes a single word into a memory instance via JTAG, without
# touching the rest of that memory's content — unlike write_mem.tcl
# (update_content_to_memory_from_file), which always overwrites the
# whole instance depth from a .mif. Used to set go_flag_addr to
# trigger a restart (see crt0.S:rv32_wait_restart) without wiping
# whatever the just-finished test wrote elsewhere in RAM.
#
# Usage: quartus_stp -t pulse_go_flag.tcl <hardware> <device> <ram_instance> <word_offset> <value>

package require ::quartus::insystem_memory_edit

if {[llength $argv] < 5} {
    puts stderr "Usage: quartus_stp -t pulse_go_flag.tcl <hardware> <device> <instance> <word_offset> <value>"
    exit 1
}
