# Dumps the full RAM IP content to a .mif via JTAG, for
# RV32_TEST_KIND: integration tests that need more than a single
# PASS/FAIL word checked.
#
# Usage: quartus_stp -t dump_ram.tcl <hardware_name> <device_name> <ram_instance> <out.mif>

package require ::quartus::insystem_memory_edit

if {[llength $argv] < 4} {
    puts stderr "Usage: quartus_stp -t dump_ram.tcl <hardware> <device> <instance> <out.mif>"
    exit 1
}
